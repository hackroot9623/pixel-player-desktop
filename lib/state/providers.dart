...
/// The OAuth handler, built from the client ID the user registered.
final spotifyAuthProvider = Provider<SpotifyAuth>((ref) {
  final settings = ref.watch(settingsProvider);
  return SpotifyAuth(clientId: settings.spotifyClientId);
});

/// An authenticated reader, or null when nothing is connected.
///
/// Rebuilt when the stored session changes. Tokens that come back from a
/// refresh are written straight to settings, because Spotify rotates the
/// refresh token and dropping the new one means signing in again.
final spotifyApiProvider = Provider<SpotifyApi?>((ref) {
  final settings = ref.watch(settingsProvider);
  if (!settings.spotifyConnected) return null;

  final api = SpotifyApi(
    auth: ref.watch(spotifyAuthProvider),
    tokens: SpotifyTokens(
      // No access token is stored: it lives an hour, so the first call just
      // refreshes. One fewer secret on disk.
      accessToken: '',
      refreshToken: settings.spotifyRefreshToken,
      expiresAt: DateTime.fromMillisecondsSinceEpoch(0),
    ),
    onTokensRefreshed: (tokens) =>
        settings.spotifyRefreshToken = tokens.refreshToken,
  );
  ref.onDispose(api.close);
  return api;
});

/// The playlists on the connected account.
final spotifyPlaylistsProvider =
    FutureProvider.autoDispose<List<SpotifyPlaylist>>((ref) async {
      final api = ref.watch(spotifyApiProvider);
      if (api == null) return const [];
      return api.playlists();
    });

/// Runs the browser handshake and stores the session.
Future<void> connectSpotify(WidgetRef ref) async {
  final settings = ref.read(settingsProvider);
  final tokens = await ref.read(spotifyAuthProvider).authorize();
  settings.spotifyRefreshToken = tokens.refreshToken;

  // Name the account from the token we already hold, rather than forcing a
  // refresh on the very next call.
  final api = SpotifyApi(
    auth: ref.read(spotifyAuthProvider),
    tokens: tokens,
    onTokensRefreshed: (fresh) =>
        settings.spotifyRefreshToken = fresh.refreshToken,
  );
  try {
    settings.spotifyAccount = await api.me() ?? '';
  } on SpotifyAuthException {
    // The session is good even if the profile call is not; leave the name blank.
  } finally {
    api.close();
  }
...
class SpotifyImportResult {
  ...
  Iterable<TrackMatch> get imported => matches.where((m) => m.matched);
  Iterable<TrackMatch> get missing => matches.where((m) => !m.matched);
  Iterable<TrackMatch> get uncertain =>
      matches.where((m) => m.confidence == MatchConfidence.loose);
}

/// Imports one Spotify playlist as a local playlist of matched tracks.
///
/// Only tracks found in the local library go in — Spotify cannot supply audio,
/// so an unmatched track would be a row that never plays. The misses come back
/// in the result for the caller to show.
Future<SpotifyImportResult> importSpotifyPlaylist(
  WidgetRef ref, {
  required String playlistId,
  required String playlistName,
  bool savedTracks = false,
}) async {
  final api = ref.read(spotifyApiProvider);
  if (api == null) {
    throw const SpotifyAuthException('Connect a Spotify account first.');
  }

  final tracks = savedTracks
      ? await api.savedTracks()
      : await api.playlistTracks(playlistId);
  // Matching runs against the local library specifically: the other sources
  // resolve their audio lazily and cannot be searched offline.
  final matches = matchTracks(tracks, ref.read(libraryProvider).songs);

  final songIds = [
    for (final match in matches)
      if (match.matched) match.song!.id,
  ];
  final created = songIds.isEmpty
      ? null
